...
function loadGradeEntry(courseId) {
    ...
    html += '<button type="button" class="btn btn-outline-danger btn-sm" id="resetGradesBtn"><i class="fas fa-undo"></i> Reset</button>';
    html += '</div></form>';
    area.innerHTML = html;
    ...
    document.getElementById('resetGradesBtn').addEventListener('click', function() {
        if (confirm('Reset all grades for this course?')) {
            var grades = getGrades();
            var toRemove = grades.filter(function(g) { return g.courseId === courseId; });
            toRemove.forEach(function(g) {
                var index = grades.indexOf(g);
...
function renderAdminView() {
    ...
    // Admin modal listeners (if modals exist)
...
// ---- ADMIN MODAL LISTENERS (optional – if your modals exist) ----
function attachAdminListeners() {
    // Add Student
    var addStudentBtn = document.getElementById('saveStudentBtn');
    if (addStudentBtn) {
        ...
        newBtn.addEventListener('click', function() {
            ...
            var program = document.getElementById('studentProgram').value.trim();
            ...
            var users = getUsers();
            if (users.find(function(u) { return u.email === email; })) {
                showErrorModal('Duplicate Email', 'Email already exists.');
                return;
            }
            var students = getStudents();
            var newId = generateId(students);
            students.push({ id: newId, name: name, email: email, program: program, level: level });
            saveStudents(students);
            users.push({ id: generateId(users), email: email, password: 'student123', role: 'student', studentId: newId, lecturerId: null });
            saveUsers(users);
            ...
            if (modal) modal.hide();
            renderAdminStudents();
            showSuccessModal('Student Added', 'Student added successfully. Password: student123');
        });
    }

    // Add Course
    var addCourseBtn = document.getElementById('saveCourseBtn');
    if (addCourseBtn) {
        var newBtn2 = addCourseBtn.cloneNode(true);
        addCourseBtn.parentNode.replaceChild(newBtn2, addCourseBtn);
        newBtn2.addEventListener('click', function() {
            var code = document.getElementById('courseCodeInput').value.trim();
            var title = document.getElementById('courseTitleInput').value.trim();
            var credit = parseInt(document.getElementById('courseCreditInput').value);
            var lecturerId = parseInt(document.getElementById('courseLecturerSelect').value);
            if (!code || !title || isNaN(credit) || !lecturerId) {
                showErrorModal('Missing Fields', 'Please fill all fields.');
                return;
            }
            var courses = getCourses();
            if (courses.find(function(c) { return c.code === code; })) {
                showErrorModal('Duplicate Code', 'Course code already exists.');
                return;
            }
            courses.push({ id: generateId(courses), code: code, title: title, credit: credit, lecturerId: lecturerId });
            saveCourses(courses);
            ...
            if (modal) modal.hide();
            renderAdminCourses();
            showSuccessModal('Course Added', 'Course added successfully.');
        });
    }

    // Enroll Student
    var enrollBtn = document.getElementById('saveEnrollBtn');
    if (enrollBtn) {
        var newBtn3 = enrollBtn.cloneNode(true);
        enrollBtn.parentNode.replaceChild(newBtn3, enrollBtn);
        newBtn3.addEventListener('click', function() {
            var studentId = parseInt(document.getElementById('enrollStudentSelect').value);
            var courseId = parseInt(document.getElementById('enrollCourseSelect').value);
            var session = document.getElementById('enrollSession').value.trim();
            var semester = parseInt(document.getElementById('enrollSemester').value);
            if (!studentId || !courseId || !session) {
                showErrorModal('Missing Fields', 'Please select all fields.');
                return;
            }
            var enrollments = getEnrollments();
            if (enrollments.find(function(e) { return e.studentId === studentId && e.courseId === courseId; })) {
                showErrorModal('Duplicate Enrollment', 'Student already enrolled.');
                return;
            }
            enrollments.push({ id: generateId(enrollments), studentId: studentId, courseId: courseId, session: session, semester: semester });
            saveEnrollments(enrollments);
            ...
            var modal = bootstrap.Modal.getInstance(document.getElementById('enrollStudentModal'));
            if (modal) modal.hide();
            renderAdminEnrollments();
    ...
    // Create Lecturer
    var lecturerBtn = document.getElementById('saveLecturerBtn');
    if (lecturerBtn) {
        var newBtn4 = lecturerBtn.cloneNode(true);
        lecturerBtn.parentNode.replaceChild(newBtn4, lecturerBtn);
        newBtn4.addEventListener('click', function() {
            var name = document.getElementById('lecName').value.trim();
            var email = document.getElementById('lecEmail').value.trim();
            var password = document.getElementById('lecPassword').value;
            var department = document.getElementById('lecDepartment').value.trim();
            ...
            var lecturers = getLecturers();
            var newLid = generateId(lecturers);
            lecturers.push({ id: newLid, name: name, email: email, department: department });
            saveLecturers(lecturers);
            users.push({ id: generateId(users), email: email, password: password, role: 'lecturer', studentId: null, lecturerId: newLid });
            saveUsers(users);
            ...
            var modal = bootstrap.Modal.getInstance(document.getElementById('createLecturerModal'));
            if (modal) modal.hide();
            renderAdminLecturers();
            renderAdminCourses();
            showSuccessModal('Lecturer Created', 'Lecturer created successfully.');
        });
    }
}    console.log('Data initialization complete!');
}
